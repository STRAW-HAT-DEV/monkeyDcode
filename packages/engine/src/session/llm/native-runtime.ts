...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@monkeydcode/llm"
import type { LLMClientShape } from "@monkeydcode/llm/route"
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @monkeydcode/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      ...
      // the @monkeydcode/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
